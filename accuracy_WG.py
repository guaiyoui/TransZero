...
if __name__ == "__main__":
    ...
    for i in tqdm(range(query_score.shape[0])):
        ...
        # query_index = torch.Tensor([query_index[0]]).int()
        ...
        # f1_score_collect.append(f1_score_calculation(y_pred[i].int(), labels[i].int()))
        # print("F1 score by of one sample: {:.4f}".format(f1_score_calculation(y_pred[i].int(), labels[i].int())))
    
    # print("F1 score by a weight gain: {:.4f}".format(mean(f1_score_collect)))
