...
class PretrainModel(nn.Module):
    ...
    def contrastive_loss(self, node_tensor, neighbor_tensor):
    ...
    def contrastive_kmeans_loss(self, node_tensor, group_connect_matrix, minus_group_connect_matrix):
        ...
        group_loss = minus_group_connect_matrix.multiply(pairwise_similary)-group_connect_matrix.multiply(pairwise_similary)
        # group_loss = group_connect_matrix.multiply(pairwise_similary)
        # group_loss = torch.abs(torch.sum(group_loss))
        group_loss = torch.sum(group_loss)
        TotalLoss += 0.000001*group_loss

        return TotalLoss
    
    def pq_loss(self, node_tensor, neighbor_tensor, adj_, minus_adj, p, q):
        ...
        pq_loss = torch.sum(torch.mul(q-p, q-p))
        TotalLoss += 0.000001*pq_loss
...
if __name__ == "__main__":
    ...
    model_kmeans = PretrainModel(input_dim=processed_features.shape[2], config=args).to(args.device)
    ...
    for epoch in range(args.epochs):
        ...
        if epoch % args.group_epoch_gap == 0:
            ...
            if epoch == 0:
                ...
                if num_communities >= 10:
                    num_communities = 10
                cluster_ids_x, center_tensors = kmeans(X=torch.Tensor(node_embedding), num_clusters=num_communities, distance='cosine', device=args.device)
        ...
        for index, item in enumerate(data_loader):
            ...
            if epoch < args.group_epoch_gap:
            # if epoch < 10 or epoch > 20:
                loss_train = model.contrastive_link_loss(node_tensor, neighbor_tensor, adj_, minus_adj)
            else:
                # print(node_tensor.shape, center_tensors.shape)
                similarity = torch.mm(node_tensor, center_tensors.to(args.device).t())
                q, p = pq_computation(similarity)
                ...
                loss_train = model.pq_loss(node_tensor, neighbor_tensor, adj_, minus_adj, p, q)
            loss_train.backward()
            optimizer.step()
            lr_scheduler.step()
